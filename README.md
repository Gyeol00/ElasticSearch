...
Elasticsearch에는 노리(Nori) 형태소 분석기를 적용해 정교한 한글 기반 검색 기능을 구현하였으며, Spring Boot 기반 REST API로 Elasticsearch 검색 기능을 클라이언트 화면에 연동했습니다. 또한, Kibana 대시보드를 통해 데이터 모니터링과 시각화가 가능합니다.
